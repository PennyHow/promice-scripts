...
for inf in infile: 
    ...
    # Get plotting years
    years = list(range(df_hour.index[0].year, 2025))
    ...
    # Plot iteratively over years  # ['/home/pho/Downloads/HUM.csv']    # ['/home/pho/Downloads/HUM.csv'
    for y in range(len(years)):
        
        # Subset dataframe to years
        date_range = [datetime.date(years[y],3,15), datetime.date(years[y],10,15)]
        doy_range = [d.timetuple()[7] for d in date_range]
        df_subset = df_hour[str(years[y])+'-03-15':str(years[y])+'-10-15']
        ...
        if df['t_u'].isna().sum() < 150:  
            ...
            ax[1].plot(df.index, df['t_cum'], c=colors[y], linestyle='-',
                       linewidth=lw, label=None)
            pddmax.append(df['t_cum'].max())
            first.append(df.loc[df['t_cum']==1].date)
    ...
    ticks = [datetime.date(years[y], 4, 1).timetuple()[7], 
    ...
    for f,p in zip(first,pddmax):
        try:
            print(str(list(f)[0])+': '+str(p)+' melt days')
        except:
            print('Skipping year')
